...
from .exceptions import IgnoredMatchException, NoMatchException, NoParentException, NoTypeException
...
def log_callback(wrapped_function):
    ...
    def _wrapper(parser, match, **kwargs):
        ...
        debug_log(f"{func_name} <- {match.group()}")

        try:
            result = wrapped_function(parser, match, **kwargs)
        except IgnoredMatchException:
            debug_log(f"{func_name} -> IGNORED")
            raise

        debug_log(f"{func_name} -> {result}")
...
class _Parser:
    ...
    def find_at_level(self, level):
        for candidate in reversed(self.seen):
            if candidate.level < level:
                return candidate
        raise NoParentException(f"Unable to find element at level {level}")
    ...
    def find_match(self):
        ...
        raise NoMatchException(f"None of the known patterns match for {self.source[self.pos :]}")
